import { VendorCheckboxFacet } from '@process-creative/ajax-collection-v3';
import {
  t
} from '@process-creative/pc-slate-tools';
import * as Language from '../../../tools/constants/language';

export class CustomVendorCheckboxFacet extends VendorCheckboxFacet {
  getTemplate(title, body) {
    let options = this.getOptionsToDraw();
    let numSelected = 0;
    options.forEach((option,i) => {
      // skip first as it is 'all'
      if(this.isSelected(option) && i > 0) {
        numSelected++;
      }
    });
